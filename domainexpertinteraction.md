...
| **HR/Compliance Manager** | The administrative role responsible for defining expense policies, managing user roles, and overseeing system governance. (System equivalent: **Configuration Manager**). |
...
| **Finance Officer** | Specialized role focused on the monetary lifecycle. Responsible for authorizing batch payments, monitoring corporate wallet balances, and resolving discrepancies during the reconciliation process. |
...
| **Configuration Manager** | Administrative person responsible for system maintenance. Handles user provisioning (onboarding/offboarding), assigns RBAC roles, and manages global system configurations. |
...
* **Steps:**  
  ...
  4. Request goes directly to the Finance Officer/HR queue.  
...
* **Steps:**  
  ...
  4. Finance Officer receives "Approved" claim and adds it to the Payment Batch.  
...
* **Trigger:** Finance Officer initiates a batch payment, but the bank API times out.  
* **Steps:**  
  ...
  4. Finance Officer triggers the "Reconciliation" module to fetch the latest Bank Feed.  
