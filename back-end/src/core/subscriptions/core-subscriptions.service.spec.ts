...
describe('CoreSubscriptionsService', () => {
  ...
  describe('findAll', () => {
    it('lists subscriptions with pagination metadata and relation filters', async () => {
      ...
      expect(mockPrisma.organizationSubscription.findMany).toHaveBeenCalledWith(
  ...
  describe('startTrial', () => {
    it('throws if organization missing', async () => {
      ...
      await expect(service.startTrial('org_1', 'plan_1', 14)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('creates trial subscription and history', async () => {
      ...
      expect(mockPrisma.subscriptionHistory.create).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('audits platform-created subscriptions', async () => {
      mockPrisma.organization.findUnique.mockResolvedValue({ id: 'org_1' });
      mockPrisma.plan.findUnique.mockResolvedValue({
        status: 'ACTIVE',
        billingInterval: BillingInterval.MONTHLY,
      ...
      await service.startTrial('org_1', 'plan_1', 14, 'staff_1');
  ...
  describe('activate', () => {
    it('throws on invalid transition', async () => {
      ...
      await expect(service.activate('sub_1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('upgrade', () => {
    it('throws if plan is missing', async () => {
      ...
      await expect(service.upgrade('sub_1', 'plan_2')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('performs upgrade', async () => {
      mockPrisma.organizationSubscription.findUnique.mockResolvedValue({
        status: SubscriptionStatus.ACTIVE,
        priceAtSubscription: 50,
        plan: { id: 'plan_1', name: 'Starter' },
      });
      mockPrisma.plan.findUnique.mockResolvedValue({
        id: 'plan_2',
        status: 'ACTIVE',
  ...
  describe('downgrade', () => {
    it('throws if target plan is more expensive', async () => {
      ...
      await expect(service.downgrade('sub_1', 'plan_2')).rejects.toThrow(
        BadRequestException,
      );
    });
