...
export function SubscriptionDetailPage() {
  ...
  return <section aria-labelledby="subscription-detail-title">
    ...
    {!loading && data && <>
      ...
      <section className="status-card form-panel section-panel" aria-labelledby="features-heading"><div className="section-heading"><div><h2 id="features-heading">Included features</h2><p>Authoritative entitlements from the current plan.</p></div></div>{(data.plan.features ?? []).length === 0 ? <p className="empty-copy">No enabled features are configured for this plan.</p> : <div className="permission-list">{(data.plan.features ?? []).map((feature) => <div className="permission-row" key={feature.id}><div><code>{feature.key}</code><p className="muted-copy">{feature.name}</p></div><span className="muted-copy">{feature.value === null ? 'Enabled' : String(feature.value)}</span></div>)}</div>}</section>
