document.addEventListener("DOMContentLoaded", function () {
  ...
  function refreshQuote() {
    ...
    var employeeCount = employeeCountInput && Number(employeeCountInput.value);
    if (
      !planId ||
    ...
    fetch(base + "/api/v1/tenant/onboarding/quotes", {
      ...
      .then(function (quote) {
        ...
        pricePreview.innerHTML =
          "<strong>Estimated recurring price</strong><div style='margin-top:8px'>Base " +
          quote.currency +
          " " +
          quote.baseAmount +
          " + employees " +
          ...
          "</div><div style='font-size:.75rem;margin-top:6px;color:var(--text-secondary)'>Included employees: " +
  ...
  fetch(base + "/api/v1/tenant/onboarding/plans")
    ...
    .then(function (data) {
      ...
      if (planSelect)
        planSelect.addEventListener("change", function () {
          ...
          var addOns = plan
            ...
            : [];
          if (addOnOptions) {
            addOnOptions.style.display = addOns.length ? "block" : "none";
            addOnOptions.innerHTML = addOns.length
              ? "<label class='form-label'>Optional add-ons</label>" +
                addOns
                  .map(function (feature) {
                    return (
                      "<label style='display:block;margin:8px 0;color:var(--text-secondary)'><input class='reg-addon' type='checkbox' value='" +
                      feature.id +
                      "'> " +
                      feature.name +
                      " (" +
      ...
      if (employeeCountInput)
        employeeCountInput.addEventListener("input", refreshQuote);
    })
  ...
  form.addEventListener("submit", function (event) {
    ...
    hideError();
    var orgName = document.getElementById("reg-org-name").value.trim();
    var orgEmail = document.getElementById("reg-org-email").value.trim();
    var slug = document
    ...
    var nameParts = document
      ...
      .value.trim()
      .split(/\s+/);
    var email = document.getElementById("reg-admin-email").value.trim();
    ...
    var employeeCount = employeeCountInput
      ...
      : 0;
    if (orgName.length < 2)
      return showError("Organization name must be at least 2 characters.");
    ...
    if (!planId) return showError("Please select an active plan.");
    if (!Number.isInteger(employeeCount) || employeeCount < 1)
      return showError("Enter a positive whole number of employees.");
    ...
    fetch(base + "/api/v1/tenant/registrations/organizations", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: orgName,
      ...
      .then(function (result) {
        ...
        var empEl = document.getElementById("success-admin-emp-id");
        var planEl = document.getElementById("success-plan");
        var statusEl = document.getElementById("success-subscription-status");
        if (nameEl) nameEl.textContent = orgName;
        if (emailEl) emailEl.textContent = orgEmail;
        if (idEl) idEl.textContent = result.organizationId;
        if (empEl) empEl.textContent = result.user.employeeId;
        if (planEl) planEl.textContent = result.subscription.plan.name;
        if (statusEl) statusEl.textContent = result.subscription.status;
      })
      .catch(function (error) {
